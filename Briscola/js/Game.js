...
class Game {
    ...
    async play(deck, pc, player, briscola) {
        ...
        while (player.cards.length > 0 || pc.cards.length > 0) {
            ...
            updateDeckCards(deck.cards.length);
    ...
    turnWinner(pcCard, playerCard, briscola, turn, player, pc) {
        ...
        if (!playerCard || !pcCard) {
            console.warn(
                "Una delle carte è undefined/null, nessun punto assegnato."
            );
            return turn;
        }
        ...
        // pc gioca briscola, player no
            ...
            pcCard.seed === briscolaSeed &&
            playerCard.seed !== briscolaSeed
        ) {
            winner = "pc";
        }
        ...
        const playerCardPoints =
            typeof playerCard.value !== "undefined"
                ? playerCard.value
                : playerCard.value;
        const pcCardPoints =
            typeof pcCard.value !== "undefined" ? pcCard.value : pcCard.value;

        if (winner === "player") {
            player.points =
                Number(player.points) +
                Number(playerCardPoints) +
                Number(pcCardPoints);
    ...
    async cleanDuelArea(duelArea) {
        ...
        if (duelArea.children.length === 2) {
            ...
            console.warn(
                `cleanDuelArea: non rimuovo, figli presenti: ${duelArea.children.length}`
            );
        }
